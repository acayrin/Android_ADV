...
class ClassAdapter(val context: Context) : BaseAdapter() {
    private val daoMonHoc: DAO_MonHoc = DAO_MonHoc(context)
    ...
    override fun getView(p0: Int, p1: View?, p2: ViewGroup?): View {
        ...
        if (view == null) {
            view = (context as Activity).layoutInflater.inflate(
                R.layout.component_list_item_class,
                p2,
                false
            )

            val tvClassCourse = view.findViewById<TextView>(R.id.lv_item_class_course)
            val tvClassDate = view.findViewById<TextView>(R.id.lv_item_class_date)
            val tvClassDesc = view.findViewById<TextView>(R.id.lv_item_class_description)
